...
router.get('/configbs',function(req,res,next){
...
router.get('/reportingconfig',function(req,res,next){
...
router.get('/defaultbs',function(req,res,next){	
		var postData = {};			
		Factory_Bootstrap.find(function(err,data){
			if(err){
				...
				if(data.length > 0){
					...
					request.post(data[0].server_uri,{form : postData},function(error,response,body){
						if(error){
							...
							Cofigured_Bootstrap.findOneAndRemove({serial : serial}, function (err,data){
								if(err){
									res.send({status : 'Factory bootstrap failed !!'});
								}
								else{									
	...
	Cofigured_Bootstrap.find(null,null,{sort : {priority : 1, created_at : -1}}, function(err,data){
		if(err){
			...
			if(data.length > 0){				
				...
				request.put(data[0].server_uri+'/'+data[0].device_id,{form : postData},function(error,response,body){
					if(error){
	...
	Cofigured_Bootstrap.find(null,null,{sort : {priority : 1, created_at : -1}}, function(err,data){
		if(err){
			...
			if(data.length > 0){				
				...
				request.del(data[0].server_uri+'/'+data[0].device_id,function(error,response,body){
					if(error){
						...
						if(data.length > 0){																		
							...
							request.get(data[0].server_uri+'/'+cbdata[0].device_id,function(error,response,body){
								if(error){
									...
									Cofigured_Bootstrap.findOneAndUpdate({serial : serial},{server_uri: server_uri,priority : priority, device_id : device_id,manufacturer : manufacturer,
										model: model,firmware : firmware,updated_at : updated_at},function(err,post){	
										if(err){
											return next(err);
										}
										else{
...
router.options('/:name/:parameter_name/:frequency',function(req,res,next){
